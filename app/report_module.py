...
class Report:
    ...
    def profit_of_ten_products_ave(self, from_month, from_year, to_month, 
                                   to_year, sort, duration, num):
        
        fitered_ordersinfo = self._filter_df_by_date(
                self.orders_partial_df, "Order Date", from_month, from_year, 
                to_month, to_year)
        ...
        fitered_ordersinfo_sum = fitered_ordersinfo[
                ["Product Name", "Profit", "Quantity", "Order Date"]]\
            .groupby(["Product Name"]).sum()
            
        fitered_ordersinfo_sum = fitered_ordersinfo_sum.reset_index()
    
        fitered_ordersinfo_sum["Average Profit/Unit"] = \
            fitered_ordersinfo_sum.loc[:, "Profit"].apply(np.float) / \
                fitered_ordersinfo_sum.loc[:, "Quantity"].apply(np.float)
            
        fitered_ordersinfo_sum = \
            fitered_ordersinfo_sum.sort_values(by="Average Profit/Unit", 
                                               ascending=sort)
            
        print("\n\n" + self.DISPLAY_LINE_STR)
        if sort and duration == 'q':
            title = (f"Least Profitable Product Report - Quarter: "
                     f"{self._change_month_to_quarter(num)} Year: {from_year}")
                    
            print(f"\t\t\t\t--------[Least Profitable Product Report]--------"
                  f"[Quarter: {self._change_month_to_quarter(num)} Year: "
                  f"{from_year}]--------\n")
            
        elif sort and duration == 'm':
            title = (f"Least Profitable Product Report - Month: {num} Year: "
                     f"{from_year}")
            ...
            title = (f"Most Profitable Product Report - Quarter: "
                     f"{self._change_month_to_quarter(num)} Year: {from_year}")
            ...
            title = (f"Most Profitable Product Report - Month: {num} Year: "
                     f"{from_year}")
        ...
        self._generate_bar_chart(chart_df, title, 90, "Product Name", 
                                 "Product Name", "Average Profit/Unit",
                                 "Average Profit/Unit")
    
    
    def active_customer_report(self, from_month, from_year, to_month, to_year,
                               sort, duration, num):
        
        customers = self.orders_full_df[["Order Date", "Customer Name", 
                                         "Profit"]]
        
        filtered_customers = \
            self._filter_df_by_date(customers, "Order Date", from_month, 
        ...
        for cust in unique_customer_list:
            customer_select = filtered_customers.loc[filtered_customers
                                                     ["Customer Name"] == cust]
            my_len = customer_select["Customer Name"].count()
            num_of_orders_dic[cust] = my_len
    
        filtered_customers_sum = \
            filtered_customers.groupby(["Customer Name"]).sum()
        filtered_customers_sum = filtered_customers_sum.reset_index()
        filtered_customers_sum['Total Num of Orders'] = \
            filtered_customers_sum['Customer Name'].map(num_of_orders_dic)
        filtered_customers_sum = \
            filtered_customers_sum.sort_values(by="Profit", ascending=sort)
        print("\n\n" + self.DISPLAY_LINE_STR)
        if sort and duration == 'q':
            ...
            print(f"\t\t\t\t--------[Least Profitable Customer Report]--------"
                  "[Quarter: {self._change_month_to_quarter(num)} Year: "
                  "{from_year}]--------\n")
    ...
    def sales_and_profits_by_region_yearly(self, year):
        sales = {}
        profits = {}
        ...
        _fig, ax = plt.subplots(figsize=(17, 6))
        ax2 = ax.twinx()
        ax.set_title(f"Sales and Profits by Region per Month vs Company Total "
                     f"per Month for the Year {year}")
    ...
    def sales_and_profits_by_region(self, from_month, from_year, to_month,
        ...
        df_groupby_sales.set_index('Region')[['Sales', 'Profit']] \
            .plot(kind='bar', ax=ax)
                                                                       
        if duration == 'm' or duration == 'M':
            plt.title(f"Sales and Profits by Region - Month: {from_month} "
                      f"Year: {from_year}")
        else:
            plt.title(f"Sales and Profits by Region - Quarter: "
                      f"{self._change_month_to_quarter(num)} Year: "
                      f"{from_year}")
            
        ax.set_xlabel("Region")
        ax.set_ylabel("Dollar Amount")
        vals = ax.get_yticks()
        ax.set_yticklabels(['${:,.0f}'.format(x).replace('$-', '-$') \
            for x in vals])
        plt.show()
    ...
    def discounts_by_region(self, from_month, from_year, to_month, to_year, 
        ...
        df['Discount'] = pd.Series(["{0:.2f}%".format(val * 100) \
            for val in df['Discount']], index=df.index)
        ...
        ax.set_yticklabels(['{:.0f}%'.format(x) for x in vals])
        if duration == 'm' or duration == 'M':
            plt.title(f"Discounts Given out by Region - Month: {from_month} "
                      f"Year: {from_year}")
        else:
            plt.title(f"Discounts Given out by Region - Quarter: "
                      f"{self._change_month_to_quarter(num)} Year: "
                      f"{from_year}")
        # fix the legend
    ...
    def discounts_by_category_and_region(self, from_month, from_year, to_month,
        ...
        filtered_region_central_df1 = region_central_df[['Order Date', 
                                                         'Category',
                                                         'Discount']]
        filtered_region_east_df1 = region_east_df[['Order Date', 'Category', 
                                                   'Discount']]
        filtered_region_south_df1 = region_south_df[['Order Date', 'Category', 
                                                     'Discount']]
        filtered_region_west_df1 = region_west_df[['Order Date', 'Category', 
                                                   'Discount']]
        
        filtered_region_central_df2 = region_central_df[['Order Date', 
                                                         'Category', 
                                                         'Sub-Category', 
                                                         'Discount']]
        filtered_region_east_df2 = region_east_df[['Order Date', 'Category', 
                                                   'Sub-Category', 'Discount']]
        filtered_region_south_df2 = region_south_df[['Order Date', 'Category', 
                                                     'Sub-Category', 
                                                     'Discount']]
        filtered_region_west_df2 = region_west_df[['Order Date', 'Category', 
                                                   'Sub-Category', 'Discount']]
        
        _fig, axs = plt.subplots(2, 4, figsize=(19,10))
        
        filtered_region_central_df1.groupby(['Category', 'Discount']).size(
    ...
    def topcust_no_disc(self, from_month, from_year, to_month, to_year, 
        ...
        _fig, ax1 = plt.subplots(figsize=(17,5))
        
        df3 = df3[:10]
        
        if duration == 'm' or duration == 'M':
            print(self.DISPLAY_LINE_STR + "\n")
            title = (f"Top 10 Customers Profits Without Discounts - Month: "
            ...
            print(self.DISPLAY_LINE_STR + "\n")
            title = (f"Top 10 Customers Profits Without Discounts - Quarter: "
                     f"{self._change_month_to_quarter(num)} Year: {from_year}")        
            print(f"\t\t--------[Top 10 Customers Profits Without Discounts]"
    ...
    def topcust_high_disc(self, from_month, from_year, to_month, to_year, 
        ...
        df3 = df3.groupby(['Customer Name', 'Discount']).sum().sort_values(
                by='Profit', ascending=True)
        df3 = df3[:10]
        text_df3 = df3.reset_index()
        text_df3['Discount'] = text_df3['Discount'].map('{:.0%}'.format)
        if duration == 'm' or duration == 'M':
            #print(DISPLAY_LINE_STR + "\n")
            title = (f"Bottom 10 Customer Profits With Discounts - Month: "
                     f"{from_month} Year: {from_year}")
            ...
            #print(DISPLAY_LINE_STR + "\n")
            title = (f"Bottom 10 Customer Profits With Discounts - Quarter: "
                     f"{self._change_month_to_quarter(num)}"
                     f" Year: {from_year}")        
            #print(f"\t\t--------[Bottom 10 Customer Profits With Discounts]"
        ...
        for a in range(0, len(labels)):
            labels[a] = '{:.0f}%'.format(float(labels[a]) * 100)
        plt.legend(reversed_handles,labels,loc='lower right', title="Discount")
        ax1.set_yticklabels(['${:,.0f}'.format(x).replace('$-', '-$') \
            for x in vals])
        plt.show()
    
    def _generate_bar_chart(self, df, title, xaxis_rotation, xaxis_df_name, 
                            xaxis_label, yaxis_df_name, yaxis_label):
    ...
    def _generate_bar_chart_with_line_twinx(self, df, title, xaxis_rotation,
                                            xaxis_df_name, xaxis_label,
                                            yaxis1_df_name, yaxis2_df_name,
                                            yaxis1_label, yaxis2_label):
        sns.set(style="whitegrid")
        _fig, ax = plt.subplots(figsize=(19, 6))
    ...
    def dashboard_report(self, from_month, from_year, to_month, to_year, sort, 
                         duration, num):
        
        metrics_df = self._filter_df_by_date(self.orders_full_df, "Order Date",
                                             from_month, from_year, to_month, 
                                             to_year)
        ...
        qty = metrics_df[['Category', 'Quantity']]\
            .groupby(['Category']).count()
        sales = metrics_df[['Category', 'Sales']].groupby(['Category']).sum()
        profit = metrics_df[['Category', 'Profit']].groupby(['Category']).sum()
        result = pd.concat([qty, sales, profit], axis=1, sort=False)\
            .reset_index()
        result['Profit Ratio'] = (result.loc[:, "Profit"] /\
              result.loc[:, "Sales"]) * 100
        result.loc[3] = ['Total'] + [result['Quantity'].sum(), 
                                     result['Sales'].sum(), 
                                     result['Profit'].sum(),
                                     result['Profit Ratio'].sum()]
        result = result.round({'Profit Ratio': 1})
        result['Sales'] = result['Sales'].astype(int).apply(
                lambda x: "{:,}".format(x))
        result['Sales'] = '$' + result['Sales'].astype(str)
    
        result['Profit'] = result['Profit'].astype(int).apply(
                lambda x: "{:,}".format(x))
    ...
    def dashboard_pie_graphs(self, from_month, from_year, to_month, to_year, 
        ...
        df = self._filter_df_by_date(self.orders_full_df, "Order Date", 
        ...
        if duration == 'Quarter':
            _from = self._change_month_to_quarter(from_month)
        else:
            _from = from_month
        
        df = df[["Category", "Sales", "Profit"]]
        
        grp = df.groupby(['Category']).sum().reset_index() \
            .set_index('Category')
        
        _fig1, (ax1, ax2) = plt.subplots(1, 2, figsize=(15,6))
        labels = df['Category'].unique()
        ax1.pie(grp['Sales'], autopct='%1.1f%%', labels=labels)
        ax1.set_title("Sales")
        ax2.pie(grp['Profit'], autopct='%1.1f%%', labels=labels)
        ax2.set_title("Profits")
        plt.suptitle(f"Sales and Profits by Category - {duration}: {_from} "
                     f"Year: {from_year}")
        plt.show()
        print(self.DISPLAY_LINE_STR + "\n")
    
    
    def _change_month_to_quarter(self, num):
        if 1 <= num <= 3:
            return 1
        elif 4 <= num <= 6:
            return 2
    ...
    def _filter_df_by_date(self, df, date_column, from_month, from_year, 
                           to_month, to_year):
        if to_month == 12:
            to_month = 1
            to_year = to_year + 1
